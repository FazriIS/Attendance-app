...
class AbsenController extends Controller
{
    ...
    public function absenMasuk(Request $request, $id)
    {
        ...
        if($request["jarak_masuk"] > $radius) {
            ...
            $waktuAwal = date('Y-m-d H:i:s', $awal);
            $waktuAkhir = date('Y-m-d H:i:s', $akhir);
            
            $awalCarbon = Carbon::parse($waktuAwal);
            $akhirCarbon = Carbon::parse($waktuAkhir);
            $status = $awalCarbon->diff($akhirCarbon);
            $waktuString = $status->format('%h:%i:%s');
            $change = Carbon::createFromTimestamp(strtotime($waktuString));
            
            $diff = $change->diffInSeconds($change->copy()->startOfDay());
            $selisihWaktu = $akhirCarbon->diffInMinutes($awalCarbon);
            
            // if($selisihWaktu <= 60){
            //     return back()->with('error', 'Not time for absent!');
            // }else if($selisihWaktu >= 120){
            //     return back()->with('error', 'its to long for absent');
            // }
    ...
    public function absenPulang(Request $request, $id)
    {
        ...
        if($request["jarak_pulang"] > $radius) {
            ...
            $pulang  = strtotime($tgl_skrg . $request["jam_pulang"]);
            $waktupulang = date('Y-m-d H:i:s', $pulang);
            $waktuAkhir = date('Y-m-d H:i:s', $akhir);
            
            
            $pulangCarbon = Carbon::parse($waktupulang);
            $akhirCarbon = Carbon::parse($waktuAkhir);
            $status = $pulangCarbon->diff($akhirCarbon);
            
            $waktuString = $status->format('%h:%i:%s');
            $change = Carbon::createFromTimestamp(strtotime($waktuString));
            $diff = $change->diffInSeconds($change->copy()->startOfDay());
            $selisihWaktu = $akhirCarbon->diffInMinutes($pulangCarbon);
    ...
    public function deleteAdmin(Request $request, $id)
    {
        ...
        $history = new History;
        $history->remark = 'Delete';
        $history->insertBy = 'Shift ' . $user->name;
        $history->inserttime = $delete->created_at;
        $history->deleteBy = auth()->user()->name;
        $history->deletetime = Carbon::now();
        $history->save();


        

        if($delete->foto_jam_absen && $delete->foto_jam_pulang == null){
            $delete->delete();
            return redirect('/data-absen')->with('success', 'Data Berhasil di Delete');
        }

        if($delete->foto_jam_absen != null && Storage::exist($delete->foto_jam_absen)){
            Storage::delete($delete->foto_jam_absen);
        }
        if($delete->foto_jam_pulang != null && Storage::exist($delete->foto_jam_pulang)){
    ...
    public function myAbsen(Request $request)
    {
        ...
        $tglskrg = date('Y-m-d');
        $id = auth()->user()->id;
        $data_absen = MappingShift::where('user_id', auth()->user()->id);
        ...
        if ($request["mulai"] && $request["akhir"]) {
            $data_absen = MappingShift::where('user_id', auth()->user()->id)->whereBetween('tanggal', [$request["mulai"], $request["akhir"]])->get();
        }
        // dd($data_absen);
